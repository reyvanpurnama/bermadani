<div>
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-800 dark:text-white flex items-center gap-2.5">
                <div class="p-2 rounded-xl bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm hover:-translate-y-1 hover:shadow-md transition-all duration-200">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <div class="p-5 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="relative flex-1">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari nama, no. anggota, unit kerja..." 
                       class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
            </div>

            <!-- Filter Status Dropdown -->
            <div class="flex items-center gap-2">
                <label class="text-xs font-semibold text-slate-500 dark:text-slate-400 shrink-0">Status Pelunasan:</label>
                <select wire:model.live="statusFilter" class="px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-white text-xs font-medium focus:ring-2 focus:ring-indigo-500">
        ...
        <!-- 1. MOBILE CARD VIEW (Visible on Mobile & Tablet < lg) -->
        <div class="block lg:hidden divide-y divide-slate-200 dark:divide-slate-800">
            @forelse($members as $m)
                ...
                <div class="p-5 space-y-3">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <h4 class="font-bold text-slate-800 dark:text-white text-base">{{ $m->name }}</h4>
                            <div class="flex items-center gap-2 mt-0.5">
                                <span class="text-xs font-mono text-slate-400">#{{ $m->nomorAnggota }}</span>
                                <span class="text-xs text-slate-500">• {{ $m->unitKerja ?? 'Unit Tidak Diisi' }}</span>
                            </div>
                        </div>
                    ...
                    <div class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 text-xs space-y-1.5">
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold text-indigo-600 dark:text-indigo-400">Rp {{ number_format($gross, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-500">Potongan Pinjaman:</span>
                            <span class="font-semibold text-rose-500">{{ $loanDeduction > 0 ? '- Rp '.number_format($loanDeduction, 0, ',', '.') : '-' }}</span>
                        </div>
                        <div class="flex justify-between pt-1 border-t border-slate-200 dark:border-slate-700 font-bold">
                            <span class="text-slate-700 dark:text-slate-200">Net Pengembalian:</span>
                            <span class="text-slate-900 dark:text-white text-sm">Rp {{ number_format($netRefund, 0, ',', '.') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-2 pt-1">
                        <button wire:click="openDetailModal({{ $m->id }})" 
                            class="px-3 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 text-xs font-semibold transition-colors inline-flex items-center gap-1">
                            <i class='bx bx-info-circle text-sm'></i>
                            <span>Rincian</span>
                        </button>

                        <button wire:click="openProcessModal({{ $m->id }})" 
                            class="px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold transition-colors inline-flex items-center gap-1 shadow-sm">
                            <i class='bx bx-credit-card text-sm'></i>
                            <span>{{ $isSettled ? 'Edit Pelunasan' : 'Proses Bayar' }}</span>
                        </button>

                        @if($isSettled)
                            <a href="{{ route('admin.members.settlement-pdf', $m->id) }}" target="_blank"
                               class="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold transition-colors inline-flex items-center gap-1 shadow-sm">
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            <td class="px-4 py-3.5 text-right">
                                ...
                                <div class="text-[10px] text-slate-400">
                                    P: {{ number_format($simpok/1000) }}k | W: {{ number_format($simwa/1000) }}k | S: {{ number_format($simsuka/1000) }}k
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3.5 text-right space-x-1">
                                <button wire:click="openDetailModal({{ $m->id }})" 
                                    class="px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 text-xs font-semibold transition-colors inline-flex items-center gap-1" title="Rincian Kalkulasi">
                                ...
                                <button wire:click="openProcessModal({{ $m->id }})" 
                                    ...
                                    <span>{{ $isSettled ? 'Edit' : 'Proses' }}</span>
                                </button>
    ...
    <!-- MODAL DETAIL RINCIAN KALKULASI -->
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-lg w-full p-6 space-y-5">
                <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
                    <h3 class="text-base font-bold text-slate-800 dark:text-white flex items-center gap-2">
                        <i class='bx bx-calculator text-indigo-600'></i>
                        Rincian Hak & Kewajiban Anggota
                    </h3>
                ...
                <div class="space-y-4">
                    <div class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 text-xs space-y-1">
                        <div class="font-bold text-slate-800 dark:text-white text-sm">{{ $selectedMember->name }}</div>
                        <div class="text-slate-400">No. Anggota: #{{ $selectedMember->nomorAnggota }} • Unit: {{ $selectedMember->unitKerja ?? '-' }}</div>
                    </div>

                    <!-- Breakdown Table -->
                    <div class="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden text-xs">
                        <div class="bg-slate-100 dark:bg-slate-800 px-3 py-2 font-bold text-slate-700 dark:text-slate-200">
                            A. RINCIAN SIMPANAN (HAK)
                        </div>
                        <div class="p-3 space-y-2">
                            <div class="flex justify-between">
                                <span class="text-slate-500">1. Simpanan Pokok:</span>
                                <span class="font-mono">Rp {{ number_format($selectedMember->simpananPokok, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">2. Simpanan Wajib:</span>
                                <span class="font-mono">Rp {{ number_format($selectedMember->simpananWajib, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">3. Simpanan Sukarela:</span>
                                <span class="font-mono">Rp {{ number_format($selectedMember->simpananSukarela, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between pt-2 border-t border-slate-200 dark:border-slate-700 font-bold text-indigo-600 dark:text-indigo-400">
                                <span>TOTAL SIMPANAN GROSS:</span>
                                <span>Rp {{ number_format($selectedMember->simpananPokok + $selectedMember->simpananWajib + $selectedMember->simpananSukarela, 0, ',', '.') }}</span>
                            </div>
                        </div>

                        <div class="bg-slate-100 dark:bg-slate-800 px-3 py-2 font-bold text-slate-700 dark:text-slate-200 border-t border-slate-200 dark:border-slate-800">
                            B. POTONGAN PINJAMAN (KEWAJIBAN)
                        </div>
                        <div class="p-3 space-y-2">
                            <div class="flex justify-between text-rose-600 dark:text-rose-400">
                                <span>Potongan Pinjaman Aktif:</span>
                                <span class="font-mono font-bold">- Rp {{ number_format($selectedSettlement ? $selectedSettlement->loan_deduction : 0, 0, ',', '.') }}</span>
                            </div>
                        </div>

                        <div class="bg-indigo-50 dark:bg-indigo-950/40 p-3 flex justify-between items-center font-extrabold text-slate-900 dark:text-white border-t border-indigo-200 dark:border-indigo-800">
                            <span>SISA BERSIH PENGEMBALIAN:</span>
                            <span class="text-base text-indigo-600 dark:text-indigo-400">Rp {{ number_format($selectedSettlement ? $selectedSettlement->net_refund_amount : ($selectedMember->simpananPokok + $selectedMember->simpananWajib + $selectedMember->simpananSukarela), 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>

                <div class="flex justify-end pt-2">
                    <button wire:click="closeModals" class="px-5 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-semibold">
                        Tutup
                    </button>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-lg w-full p-6 space-y-5">
                ...
                <form wire:submit.prevent="processSettlement" class="space-y-4">
                    <div>
                        ...
                        <select wire:model.live="payment_method" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-white text-xs font-medium focus:ring-2 focus:ring-indigo-500">
                    ...
                    @if($payment_method === 'BANK_TRANSFER')
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="text" wire:model.defer="bank_name" placeholder="Contoh: Bank BCA" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-white text-xs">
                            ...
                            <div>
                                ...
                                <input type="text" wire:model.defer="bank_account_number" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-white text-xs font-mono">
                            ...
                            <div class="col-span-2">
                                ...
                                <input type="text" wire:model.defer="bank_account_holder" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-white text-xs">
                    ...
                    <div>
                        ...
                        <input type="date" wire:model.defer="settled_at" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-white text-xs">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Catatan / Keterangan</label>
                        <textarea wire:model.defer="notes" rows="2" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-white text-xs"></textarea>
                    </div>

                    <div class="flex justify-end gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
                        <button type="button" wire:click="closeModals" class="px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-xs font-semibold">
                            Batal
                        </button>
                        <button type="submit" class="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-sm flex items-center gap-1.5">
