    ...
    @if($showDetailModal && $selectedBatch)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
            <div
                ...
                <div class="flex justify-between items-center px-6 py-4 border-b border-slate-200 dark:border-slate-700 @if($selectedBatch->status === 'REQUESTED') bg-blue-50 dark:bg-blue-500/10 @endif">
                            ...
                            <div class="flex justify-between items-end mb-2">
                                <div>
                                    ...
                                    <p class="text-[10px] text-slate-500">{{ $selectedBatch->totalSoldQty }} dari
                                        {{ $selectedBatch->totalInitialQty }} pcs terjual</p>
                                </div>
                                <span
    ...
    @if($showReceiveForm && $selectedBatch)
        <div class="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
            <div class="bg-white dark:bg-darkCard rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden animate-fade-in-up my-8">
                <div class="flex justify-between items-center px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-blue-50 dark:bg-blue-500/10">
                    <div>
                        <h3 class="font-bold text-lg text-slate-800 dark:text-white flex items-center gap-2">
                ...
                <div class="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
                    <div class="bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/30 rounded-lg p-3">
                        <p class="text-[12px] text-blue-700 dark:text-blue-400">
                            <i class='bx bx-info-circle mr-1'></i>
                            Sesuaikan jumlah barang yang diterima jika berbeda dari yang diminta. Berikan catatan untuk barang yang rusak atau tidak layak dijual.
                        </p>
                    </div>
                        ...
                        @foreach($receiveItems as $index => $item)
                            ...
                            <div class="bg-slate-50 dark:bg-slate-800 p-4 rounded-lg border {{ $isDifferent ? 'border-amber-300 dark:border-amber-500/50' : 'border-slate-200 dark:border-slate-700' }}">
                                <div class="flex justify-between items-start mb-3">
                                    ...
                                    @if($isDifferent)
                                        <span class="text-[10px] font-bold px-2 py-0.5 rounded {{ $diff > 0 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' : 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400' }}">
                                            {{ $diff > 0 ? '+' : '' }}{{ $diff }} pcs
                                        </span>
                                    @endif
                                </div>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div>
                                        <label class="text-[11px] font-bold text-slate-600 dark:text-slate-400 mb-1 block">Qty Diterima:</label>
                                    ...
                                    <div>
                                        <label class="text-[11px] font-bold text-slate-600 dark:text-slate-400 mb-1 block">Catatan:</label>
                    ...
                    <div>
                        <label class="text-[11px] font-bold text-slate-600 dark:text-slate-400 mb-1 block">Catatan Umum (Opsional):</label>
