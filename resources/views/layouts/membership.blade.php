...
<head>
    ...
    <title>@yield('title', 'Member Area') - {{ coop_config('short_name') }}</title>
        ...
        tailwind.config = {
            ...
            theme: {
                extend: {
                    ...
                    colors: {
                        primary: '{{ coop_config('theme.membership') }}', // Indigo
    ...
    <div
        ...
        <div class="flex justify-around items-center max-w-md mx-auto lg:max-w-4xl px-2 py-2">
            ...
            <a href="{{ route('membership.dashboard') }}"
                class="flex flex-col items-center gap-1 p-2 rounded-xl transition-all {{ request()->routeIs('membership.dashboard') ? 'text-primary' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300' }}">
                <i
                    class='bx {{ request()->routeIs('membership.dashboard') ? 'bxs-home-smile' : 'bx-home-smile' }} text-2xl'></i>
                <span class="text-[10px] font-medium">Beranda</span>
            </a>

            <!-- History -->
            <a href="{{ route('membership.history') }}"
                class="flex flex-col items-center gap-1 p-2 rounded-xl transition-all {{ request()->routeIs('membership.history*') ? 'text-primary' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300' }}">
                <i
                    class='bx {{ request()->routeIs('membership.history*') ? 'bxs-receipt' : 'bx-receipt' }} text-2xl'></i>
            ...
            <!-- Promotion / Shop -->
            <a href="#"
                class="flex flex-col items-center gap-1 p-2 rounded-xl transition-all {{ request()->routeIs('membership.shop') ? 'text-primary' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300' }}">
                <i class='bx {{ request()->routeIs('membership.shop') ? 'bxs-store' : 'bx-store' }} text-2xl'></i>
                <span class="text-[10px] font-medium">Belanja</span>
            </a>

            <!-- Profile -->
            <a href="{{ route('membership.profile') }}"
                class="flex flex-col items-center gap-1 p-2 rounded-xl transition-all {{ request()->routeIs('membership.profile*') ? 'text-primary' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300' }}">
                <i
                    class='bx {{ request()->routeIs('membership.profile*') ? 'bxs-user-circle' : 'bx-user-circle' }} text-2xl'></i>
