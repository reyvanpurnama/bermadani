<aside id="main-sidebar" x-data="{ 
        ...
        ratDropdown: localStorage.getItem('sidebar_rat_dropdown') === 'true' || false,
        toggleRatDropdown() {
            ...
            @if(auth()->user()->isSuperAdmin() || auth()->user()->isDeveloper() || auth()->user()->isAdmin())
                <div class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <a href="{{ route('admin.settings') }}"
                        class="nav-item flex items-center px-2 py-1.5 rounded-md transition-all group whitespace-nowrap {{ request()->routeIs('admin.settings') ? 'bg-indigo-50 dark:bg-indigo-500/10 text-primary dark:text-indigo-400 font-semibold' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white' }}">
                        <i class='bx bx-slider-alt text-sm mr-2 opacity-70 group-hover:opacity-100'></i>
                        <span class="sidebar-text text-xs font-medium transition-opacity duration-300">Pengaturan Koperasi</span>
                    </a>
                    ...
                    <a href="{{ route('admin.users') }}"
                        ...
                        <span class="sidebar-text text-xs font-medium transition-opacity duration-300">Users & Access</span>
                    </a>
                </div>
            @endif
        @endif
