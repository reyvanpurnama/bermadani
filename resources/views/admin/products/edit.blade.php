...
@section('content')
    <div class="max-w-3xl">
        ...
        <div class="mb-6">
            ...
            <p class="text-[11px] text-slate-500 ml-11">Update informasi produk {{ $product->name }}.</p>
        </div>
        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST"
            ...
            <div class="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-700">
                ...
                <div class="space-y-4" x-data="{
                            mode: 'manual',
                            buyDisplay: '{{ old('buyPrice', $product->buyPrice) ? number_format(old('buyPrice', $product->buyPrice), 0, ',', '.') : '' }}',
                            buyRaw: '{{ old('buyPrice', $product->buyPrice) }}',
                            sellDisplay: '{{ old('sellPrice', $product->sellPrice) ? number_format(old('sellPrice', $product->sellPrice), 0, ',', '.') : '' }}',
                            sellRaw: '{{ old('sellPrice', $product->sellPrice) }}',
                            percent: 30,
                            ...
                            get isProfitInvalid() {
                                return this.mode === 'profit' && this.percent >= 100;
                            }
                        }">
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <button type="button" @click="if(!['markup','profit'].includes(mode)) setMode('markup')"
                                class="flex-1 py-2 px-3 text-xs font-medium rounded-lg border-2 transition-all"
                                :class="['markup', 'profit'].includes(mode) ? 'bg-primary text-white border-primary' : 'bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-600 hover:border-primary'">
                                Margin
                            </button>
                        </div>

                        <!-- Sub Options for Margin -->
                        <div x-show="['markup', 'profit'].includes(mode)" x-transition
                            class="mt-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
                            <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <div class="relative flex items-center justify-center">
                                        <input type="radio" name="margin_type" :checked="mode === 'markup'"
                                            @change="setMode('markup')"
                                            class="peer appearance-none h-4 w-4 border-2 border-slate-300 rounded-full checked:border-primary checked:bg-primary transition-all">
                                        <div
                                            class="absolute w-2 h-2 rounded-full bg-white opacity-0 peer-checked:opacity-100 transition-opacity">
                                        </div>
                                    </div>
                                    <span
                                        class="text-xs font-medium text-slate-700 dark:text-slate-300 group-hover:text-primary transition-colors">Markup
                                        % (Dari Modal)</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <div class="relative flex items-center justify-center">
                                        <input type="radio" name="margin_type" :checked="mode === 'profit'"
                                            @change="setMode('profit')"
                                            class="peer appearance-none h-4 w-4 border-2 border-slate-300 rounded-full checked:border-primary checked:bg-primary transition-all">
                                        <div
                                            class="absolute w-2 h-2 rounded-full bg-white opacity-0 peer-checked:opacity-100 transition-opacity">
                                        </div>
                                    ...
                                    <span
                                        class="text-xs font-medium text-slate-700 dark:text-slate-300 group-hover:text-primary transition-colors">Profit
                                        % (Dari Jual)</span>
                                </label>
                            </div>
                            <p class="text-[10px] text-slate-400 mt-2"
                                x-text="mode === 'markup' ? 'Menghitung harga jual dengan menambahkan persentase markup ke harga beli.' : 'Menghitung harga jual agar mendapatkan persentase profit margin dari harga akhir.'">
                            </p>
                        </div>

                        <p class="text-[10px] text-slate-400 mt-1.5" x-show="mode === 'manual'">Input harga jual secara
                            langsung tanpa kalkulasi otomatis.</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div x-show="mode !== 'manual'" x-transition>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                    <span x-text="mode === 'markup' ? 'Persentase Markup' : 'Persentase Profit'"></span> (%)
                                </label>
                                <div class="relative">
                                    <input type="number" x-model="percent" @input="recalculate()" min="1"
                                        :max="mode === 'profit' ? 99 : 1000" step="1"
                                        class="w-full bg-slate-50 dark:bg-slate-800 border text-slate-900 dark:text-white text-sm rounded-lg px-4 py-2.5 outline-none focus:ring-2 focus:ring-primary transition-all"
                                        :class="isProfitInvalid ? 'border-rose-500 ring-2 ring-rose-500/20' : 'border-slate-200 dark:border-slate-600'"
                                        placeholder="30">
                                    <span class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 text-sm">%</span>
                                </div>
                                {{-- Profit Invalid Alert --}}
                                <div x-show="isProfitInvalid" x-transition
                                    class="mt-2 p-2.5 bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-700 rounded-lg">
                                    <p
                                        class="text-xs text-rose-600 dark:text-rose-400 font-medium flex items-center gap-1.5">
                                        <i class='bx bx-error-circle'></i>
                                        Profit tidak boleh 100% atau lebih (hasil = tak terhingga)
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {{-- Stok Saat Ini --}}
                        <div x-data="{
                            display: '{{ old('stock', $product->stock) ? number_format(old('stock', $product->stock), 0, ',', '.') : '' }}',
                            raw: '{{ old('stock', $product->stock) }}',
                            format(e) {
                                let val = e.target.value.replace(/\D/g, '');
                                this.raw = val;
                            ...
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Stok Saat Ini
                            ...
                            @error('stock')
                                <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                            @enderror
                            <p class="text-xs text-slate-400 mt-1">Perubahan stok akan dicatat sebagai penyesuaian manual
                            </p>
                        </div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Ambang Batas
                                Stok
                                <span class="text-rose-500">*</span></label>
                            <input type="number" name="threshold" value="{{ old('threshold', $product->threshold) }}"
                                required min="1"
