    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <div class="flex gap-2">
            <a href="{{ route('supplier.restock') }}"
                class="hidden sm:flex bg-white dark:bg-darkCard text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors items-center gap-2">
                <i class='bx bx-history'></i>
                Riwayat Stok
            </a>
            <a href="{{ route('supplier.products.create') }}"
                class="bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
        ...
        <div class="p-4 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row gap-4 justify-between">
            <div class="relative w-full sm:w-64">
                ...
                <input type="text" placeholder="Cari produk..."
                    class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-sm rounded-lg pl-10 pr-4 py-2 outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
            </div>
            <div class="flex gap-2">
                <select
                    class="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-sm rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary cursor-pointer">
        ...
        <div class="sm:hidden divide-y divide-slate-100 dark:divide-slate-700">
            @forelse($products as $product)
                ...
                <div class="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <div class="flex items-start gap-3">
                        ...
                        @else
                            <div
                                class="w-14 h-14 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-400 flex-shrink-0">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0">
                                    ...
                                    <p class="text-[11px] text-slate-500">{{ $product->sku }} ·
                            ...
                            <div class="flex items-center justify-between mt-2">
                                <span class="font-bold text-slate-900 dark:text-white">Rp
                                    {{ number_format($product->buyPrice, 0, ',', '.') }}</span>
                                @if($product->approvalStatus === 'PENDING')
                                    <span
                                        class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">
                                        <i class='bx bx-time-five'></i> Menunggu
                                    </span>
                                @elseif($product->approvalStatus === 'REJECTED')
                                    <span
                                        class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-400">
                                        <i class='bx bx-x-circle'></i> Ditolak
                                    </span>
                                @elseif($product->approvalStatus === 'APPROVED')
                                    @if($product->stock > 0)
                                        <span
                                            class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400">
                                            <i class='bx bx-store'></i> Stok: {{ $product->stock }}
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400">
            ...
            @empty
                <div class="p-8 text-center text-slate-500 dark:text-slate-400">
                    ...
                    <a href="{{ route('supplier.products.create') }}" class="text-primary hover:underline mt-1 text-sm">Tambah
                        produk pertama Anda</a>
                </div>
            @endforelse
        </div>
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    @else
                                        <div
                                            class="w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-400">
                            ...
                            <td class="px-6 py-4 text-sm">
                                <span class="font-medium text-slate-900 dark:text-white">Rp
                                    {{ number_format($product->buyPrice, 0, ',', '.') }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if($product->approvalStatus === 'APPROVED')
                                    @if($product->stock > 0)
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $product->stock > 10 ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400' }}">
                                            {{ $product->stock }}
                                        </span>
                                    @else
                                        <span
                            ...
                            <td class="px-6 py-4 text-center">
                                @if($product->approvalStatus === 'PENDING')
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">
                                        <i class='bx bx-time-five text-sm'></i> Menunggu Review
                                    </span>
                                @elseif($product->approvalStatus === 'REJECTED')
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-400 cursor-help"
                                ...
                                @elseif($product->approvalStatus === 'APPROVED')
                                    @if($product->stock > 0)
                                        <span
                                            class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400">
                                            <i class='bx bx-store text-sm'></i> Aktif
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400">
                                            <i class='bx bx-info-circle text-sm'></i> Stok Habis
                                        </span>
                                    @endif
                                @else
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-400">
                                        Non-aktif
                                    </span>
                                @endif
                            </td>
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-slate-500 dark:text-slate-400">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p>Belum ada produk.</p>
                                    <a href="{{ route('supplier.products.create') }}"
                                        class="text-primary hover:underline mt-1 text-sm">Tambah produk pertama Anda</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
