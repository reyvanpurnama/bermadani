# PROMPT INFOGRAFIS LEMBAR 1: DASHBOARD KINERJA KEUANGAN RAT 2025
**Ukuran Kertas**: A4 Portrait (Ratio 1:1.414)
**Desain & Tema**: Modern Corporate Emerald-Gold, Clean Infographic, Professional Dashboard Layout.

---

### [TEKS BISA DI-COPY LANGSUNG KE CHATGPT / DALL-E / CANVA AI]

```text
Buatkan infografis visual berukuran A4 Portrait untuk Laporan Keuangan Rapat Anggota Tahunan (RAT) Koperasi Bermadani.
Desain harus terlihat modern, sangat bersih, profesional, elegan dengan tema warna Emerald Green, Teal, Gold Accent, dan Clean White Background.

[HEADER ATAS]
- Logo Koperasi di sudut kiri atas dengan nama: "KOPERASI BERMADANI"
- Judul Utama: "DASHBOARD KINERJA KEUANGAN TAHUN 2025"
- Sub-judul: "Ringkasan Laporan Pertanggungjawaban Pengurus untuk Rapat Anggota Tahunan (RAT)"
- Slogan: "Bersama Anggota, Koperasi Kuat, Manfaat Nyata"

[SECTION 1: CARDS KPI UTAMA (Top Grid - 4 Card)]
1. CARD TOTAL SIMPANAN:
   - Icon: Wallet / Coin Stack
   - Angka: "Rp 222.440.000"
   - Label: "Total Dana Simpanan Anggota"
2. CARD LABA BERSIH (SHU 2025):
   - Icon: Trending Up / Chart
   - Angka: "Rp 30.499.118"
   - Label: "Total Laba Bersih Tahun 2025"
3. CARD SHU DIBAGIKAN:
   - Icon: Gift / Handshake
   - Angka: "Rp 15.000.000"
   - Label: "Total SHU Dibagikan ke Anggota (49,18%)"
4. CARD KEANGGOTAAN:
   - Icon: Users / People
   - Angka: "113 Anggota"
   - Label: "Anggota Aktif Berhak SHU (Dari 145 Total)"

[SECTION 2: VISUALISASI GRAFIK & DIAGRAM (4 Quadrant Grid)]
- QUADRANT 1 (Komposisi Simpanan Anggota - Pie/Donut Chart):
  * Simpanan Wajib: Rp 176.950.000 (79.5%)
  * Simpanan Pokok: Rp 28.500.000 (12.8%)
  * Simpanan Sukarela: Rp 16.990.000 (7.6%)
  * Catatan: "Simpanan wajib menjadi porsi utama partisipasi modal anggota."

- QUADRANT 2 (Perbandingan Arus Kas 2025 - Bar Chart):
  * Total Penerimaan Kas: Rp 168.049.500
  * Total Pengeluaran Kas: Rp 137.550.382
  * Surplus Kas Bersih: Rp 30.499.118

- QUADRANT 3 (Alokasi Laba Bersih 2025 - Donut Chart):
  * Modal Usaha Ditahan: Rp 15.499.118 (50,82%)
  * SHU Dibagikan Anggota: Rp 15.000.000 (49,18%)
  * Catatan: "Alokasi modal ditahan disiapkan untuk penguatan operasional bulan depan."

- QUADRANT 4 (Indikator Kesehatan Koperasi - Gauge Status):
  * Kecukupan Modal: BAIK & SEHAT (Hijau)
  * Likuiditas & Kas: SANGAT BAIK (Hijau)
  * Efisiensi Operasional: EFISIEN (Hijau)
  * Kualitas Pengelolaan: BAIK (Hijau)

[FOOTER BANNER]
- Keterangan: "Data bersumber dari Sistem Pembukuan Resmi Koperasi Bermadani Per 31 Desember 2025."
- Pengurus Koperasi: Ridlo Abdillah (Ketua Koperasi)
- Tanggal Pelaksanaan RAT: 31 Juli 2026
```
